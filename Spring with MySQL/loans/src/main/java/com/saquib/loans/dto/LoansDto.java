...
@Schema(name="Loans", description="Schema to hold the Loan information")
@Data
public class LoansDto {
    @NotEmpty(message="Mobile Number can not be a null or empty")
    @Pattern(regexp = "(^$|[0-9]{10})", message = "Mobile Number must be 10 digits")
    @Schema(description = "Mobile Number of Customer", example="8978954379")
    private String mobileNumber;

    @NotEmpty(message="Loan Number can not be a null or empty")
    @Pattern(regexp = "(^$|[0-9]{12})", message = "Loan Number must be 10 digits")
    @Schema(description = "Loan Number of the customer", example="123456789876")
    ...
    @Positive(message = "Total loan amount should be greater that zero")
